...
<div class="p-4" x-data="{ type: 'nationale' }"> <!-- Initialiser avec 'nationale' par défaut -->
    ...
    <form wire:submit.prevent="createPublication" class="grid grid-cols-1 sm:grid-cols-2 gap-4" enctype="multipart/form-data">
        <!-- Title Input -->
        <div>
            <label for="lab" class="block text-sm font-medium">Lab d'accueil</label>
            <input type="text" id="lab" wire:model.defer="lab" class="input input-bordered bg-white w-full" />
            @error('lab') 
        ...
        <div class="flex justify-end sm:col-span-2">
            ...
            <x-primary-button type="button" class="btn-sm" wire:click="createPublication" wire:loading.attr="disabled">
                <span wire:loading.remove wire:target="createPublication" class="mr-2">{{ __('Ajouter') }}</span>
                <x-mary-loading class="loading" wire:loading wire:target="createPublication">
                </x-mary-loading>
            </x-primary-button>
        </div>
    </form>
