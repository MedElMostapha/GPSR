...
new class extends Component {
    ...
    public function toggleValidation(User $user){
        ...
        $this->profile->update(['isValidated' => !$isValidated]);
        ...
        @else
            ...
            <div class="flex items-center space-x-6">
                ...
                <div class="flex-shrink-0">
                    @if($profile->image)
                        <img class="h-24 w-24 rounded-full object-cover" src="{{ asset('storage/' . str_replace('public/', '', $profile->image)) }}" alt="{{ $profile->name }}">
                    @else
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <span class="inline-block bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded-full">{{ $profile->roles->map(fn ($role) => $role->name)->implode(', ') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <div class="mt-2 space-x-4">
                    ...
                    @if($profile->website)
                        <a href="{{ $profile->website }}" class="text-blue-500 hover:text-blue-700">
                            <i class="fas fa-globe"></i> Website
                        </a>
                    @endif
                    @if($profile->facebook)
                        <a href="{{ $profile->facebook }}" class="text-blue-500 hover:text-blue-700">
                            <i class="fab fa-facebook"></i> Facebook
                        </a>
                    @endif
                    @if($profile->twitter)
                        <a href="{{ $profile->twitter }}" class="text-blue-500 hover:text-blue-700">
                            <i class="fab fa-twitter"></i> Twitter
                        </a>
                    @endif
                    @if($profile->linkedin)
                        <a href="{{ $profile->linkedin }}" class="text-blue-500 hover:text-blue-700">
                            <i class="fab fa-linkedin"></i> LinkedIn
                        </a>
                    @endif
                    @if($profile->github)
                        <a href="{{ $profile->github }}" class="text-blue-500 hover:text-blue-700">
            ...
            <div class="mt-6">
                ...
                <div class="mt-2 text-gray-600 space-y-2">
                    <p><span class="font-medium">Nombre de publications:</span> {{ $profile->numberOfPublicationsPublished() }}</p>
                </div>
            </div>
