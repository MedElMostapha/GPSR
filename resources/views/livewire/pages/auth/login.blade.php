    ...
    public function login(): void
    {
        $this->validate();
            ...
            if($user != null){
                ...
                session()->flash('message', 'Desolé, votre compte n\'existe pas. Veuillez contacter l\'administrateur.');
            }
...
<div>
    ...
    <div x-data="{ show: true }"
        ...
        <button @click="show = false"
            class="text-white bg-transparent hover:bg-yellow-700 rounded-full p-1">
        ...
        <div class="flex items-center justify-end mt-4">
            ...
            <x-primary-button class="ms-3"
                ...
                <x-mary-loading wire:loading>
                    {{--
                    <x-icon name="loading"
                        class="animate-spin h-5 w-5" /> <!-- Mary UI spinner --> --}}
                </x-mary-loading>
            </x-primary-button>

            <a href="{{ route('register') }}"
                class="ms-3 text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                wire:navigate>
                {{ __('Register') }}
            </a>
        </div>
